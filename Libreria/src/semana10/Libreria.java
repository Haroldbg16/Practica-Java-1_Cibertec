...
public class Libreria extends JFrame implements ActionListener {
	...
	protected void actionPerformedBtnProcesar(ActionEvent arg0) {
		...
		double pre = calcularPrecio(marca);
		double impC = calcularImporteCompra(cant,pre);
		double impD = calcularImporteDescuento(cant,impC);
		double impP = impC-impD;
		int lapiceros = calcularLapiceros(marca,cant);
		
		txtS.setText("REPORTE DE LIBRERÍA \n\n");
	...
	double calcularPrecio(String marca) {
		if(marca == "Standford") {return 4.85;}
		else if (marca == "Alpha") {return 4.35;}
		else if (marca == "Justus") {return 3.50;}
		else {return 4.55;}
	}
	double calcularImporteCompra (int cant,double pre) {
		return pre*cant;
	}
	double calcularImporteDescuento(int cant, double impC) {
		if(cant<36) { return impC *0.135;}
		else if (cant<=24 && cant<36) {return impC*0.115;}
		else if (cant<=12 && cant<24) {return impC*0.95;}
		else { return impC*0.75;}		
	}
	int calcularLapiceros(String marca,int cant){
